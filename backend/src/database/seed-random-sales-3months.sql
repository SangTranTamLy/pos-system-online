-- Seed du lieu ban hang 3 thang cho 3 nhan vien, co ca lam that.
--
-- File nay tao them du lieu cho cac bang:
-- - shifts: ca sang / chieu / toi, da dong ca
-- - orders: don hang hoan thanh, co gan shift_id
-- - order_details: chi tiet san pham da ban
-- - payments: thanh toan thanh cong
-- - audit_logs: dang nhap, mo ca, ban hang/giam gia, dong ca, dang xuat
-- - stock_transactions: xuat kho cho san pham co quan ly ton kho
-- - products: tru ton kho va tat is_available neu het hang
--
-- Cach dung:
-- 1) Chon database truoc khi chay:
--    USE quickserve_pos;
-- 2) Neu can, sua cac bien cau hinh ben duoi.
-- 3) Chay toan bo file trong MySQL Workbench.
--
-- Luu y:
-- - Script chi THEM du lieu moi, khong xoa du lieu cu.
-- - Don seed co note bat dau bang "SEED_3M_RANDOM_SALES".
-- - Ca seed co closing_note bat dau bang "SEED_3M_RANDOM_SHIFTS".
-- - Neu chay nhieu lan, du lieu se duoc cong them nhieu lan.
-- - Neu xoa seed, ton kho san pham da bi tru se KHONG tu hoan lai.
--
-- Goi y xoa du lieu seed theo thu tu:
-- DELETE al
-- FROM audit_logs al
-- LEFT JOIN orders o ON al.target_object = CONCAT('Don #', o.id)
-- LEFT JOIN shifts s ON al.target_object = CONCAT('Ca lam #', s.id)
-- WHERE o.note LIKE 'SEED_3M_RANDOM_SALES%'
--    OR s.closing_note LIKE 'SEED_3M_RANDOM_SHIFTS%'
--    OR al.description LIKE '%SEED_3M_RANDOM%';
--
-- DELETE st
-- FROM stock_transactions st
-- WHERE st.note LIKE '%SEED_3M_RANDOM_SALES%';
--
-- DELETE FROM orders WHERE note LIKE 'SEED_3M_RANDOM_SALES%';
-- DELETE FROM shifts WHERE closing_note LIKE 'SEED_3M_RANDOM_SHIFTS%';

SET @seed_end_date = CURDATE();
SET @seed_start_date = DATE_SUB(@seed_end_date, INTERVAL 92 DAY);
SET @seed_order_count = 900;
SET @seed_manager_id = '00000000-0000-0000-0000-000000000101';
SET @old_sql_safe_updates = @@SQL_SAFE_UPDATES;
SET SQL_SAFE_UPDATES = 0;

DROP PROCEDURE IF EXISTS seed_random_sales_3months;

DELIMITER $$

CREATE PROCEDURE seed_random_sales_3months()
BEGIN
  ...
  DECLARE v_days_range INT DEFAULT 1;
  ...
  DROP TEMPORARY TABLE IF EXISTS tmp_seed_employees;
  CREATE TEMPORARY TABLE tmp_seed_employees (
    seq INT AUTO_INCREMENT PRIMARY KEY,
    id CHAR(36) NOT NULL,
    full_name VARCHAR(120) NOT NULL
  );

  INSERT INTO tmp_seed_employees (id, full_name)
  SELECT id, full_name
  FROM users
  WHERE id IN (
  ...
  SET v_days_range = GREATEST(DATEDIFF(@seed_end_date, @seed_start_date), 1);

  DROP TEMPORARY TABLE IF EXISTS tmp_seed_shifts;
  CREATE TEMPORARY TABLE tmp_seed_shifts (
    seq INT AUTO_INCREMENT PRIMARY KEY,
  ...
  WHILE v_day_index <= v_days_range DO
    SET v_work_date = DATE_ADD(@seed_start_date, INTERVAL v_day_index DAY);
    SET v_shift_index = 0;
    ...
    WHILE v_shift_index < 3 DO
      ...
      WHERE seq = ((v_day_index + v_shift_index) MOD v_employee_count) + 1
      LIMIT 1;

      SET v_shift_id = UUID();

      IF v_shift_index = 0 THEN
        SET v_shift_start = TIMESTAMP(v_work_date, '06:00:00');
        SET v_shift_end = TIMESTAMP(v_work_date, '11:30:00');
      ELSEIF v_shift_index = 1 THEN
        SET v_shift_start = TIMESTAMP(v_work_date, '11:30:00');
        SET v_shift_end = TIMESTAMP(v_work_date, '17:00:00');
      ELSE
        SET v_shift_start = TIMESTAMP(v_work_date, '17:00:00');
        SET v_shift_end = TIMESTAMP(v_work_date, '22:00:00');
      END IF;

      SET v_actual_start = DATE_ADD(v_shift_start, INTERVAL FLOOR(RAND() * 12) MINUTE);
      SET v_actual_end = DATE_ADD(v_shift_end, INTERVAL FLOOR(RAND() * 18) MINUTE);
      ...
      VALUES (
        ...
        0,
        0,
        'SEED_3M_RANDOM_SHIFTS | Ca da dong tu dong de phuc vu bao cao AI',
        v_actual_start,
        v_actual_end
      );

      INSERT INTO tmp_seed_shifts (shift_id, employee_id, employee_name, shift_start, shift_end)
      ...
      (
        ...
        'DANG_NHAP',
        'Thiet bi POS',
        'SEED_3M_RANDOM_SHIFTS | Nhan vien dang nhap truoc khi mo ca.',
        NULL,
        NULL
      ),
      (
        ...
        CONCAT('SEED_3M_RANDOM_SHIFTS | Mo ca. Tien dau ca: ', FORMAT(v_opening_cash, 0), 'd.'),
        NULL,
        NULL
      );
  ...
  SELECT COUNT(*) INTO v_shift_count FROM tmp_seed_shifts;
  ...
  WHILE v_order_index < @seed_order_count DO
    ...
    FROM tmp_seed_shifts
    WHERE seq = (v_order_index MOD v_shift_count) + 1
    LIMIT 1;
    ...
    VALUES (
      ...
      0,
      0,
      CONCAT('SEED_3M_RANDOM_SALES | Nhan vien: ', v_employee_name),
      v_order_time,
      v_order_time
    );
    ...
    WHILE v_line_index < v_line_count DO
      ...
      IF v_is_tracked_stock = 1 THEN
        ...
        VALUES (
          ...
          'export',
          v_quantity,
          CONCAT('SEED_3M_RANDOM_SALES | Ban hang tai quay - don ', v_order_id, ' - ', v_product_name),
          v_order_time
        );
      END IF;
  ...
  JOIN tmp_seed_shifts tss ON tss.shift_id = s.id
  SET
  ...
  SELECT
    ...
    CONCAT('Ca lam #', s.id),
    CONCAT(
      'SEED_3M_RANDOM_SHIFTS | Dong ca. Doanh thu: ',
  ...
  WHERE s.closing_note LIKE 'SEED_3M_RANDOM_SHIFTS%';
  ...
  SELECT
    ...
    'DANG_XUAT',
    'He thong',
    'SEED_3M_RANDOM_SHIFTS | Nhan vien dang xuat sau khi dong ca.',
    NULL,
    NULL
  FROM shifts s
  JOIN tmp_seed_shifts tss ON tss.shift_id = s.id
  ...
  SELECT
    'Da seed du lieu ban hang, ca lam, thanh toan va nhat ky trong 3 thang' AS message,
    @seed_start_date AS start_date,
    @seed_end_date AS end_date,
    @seed_order_count AS created_orders,
    (SELECT COUNT(*) FROM tmp_seed_shifts) AS created_shifts,
    'orders, order_details, payments, shifts, audit_logs, stock_transactions, products.stock_quantity' AS affected_features;
END$$

DELIMITER ;

CALL seed_random_sales_3months();
