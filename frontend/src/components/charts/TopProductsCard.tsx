  ...
  return (
    <div className="rounded-3xl border border-slate-200/60 bg-white p-6 shadow-lg shadow-slate-200/40">
      <div className="mb-6 flex items-center justify-between">
        <h4 className="font-['Plus_Jakarta_Sans',sans-serif] font-black text-[14px] uppercase tracking-wider text-[#2a1b14]">
          TOP BÁN CHẠY
        </h4>
      </div>
        ...
        {products.length > 0 ? (
          products.map((product, index) => (
            <div key={product.name} className="flex items-center gap-4 py-4 border-b border-slate-100 last:border-0 last:pb-0 first:pt-0">
              <span className="w-6 text-center font-black text-[#9d4300] text-lg">{index + 1}</span>
              ...
              <div className="flex-1 min-w-0">
                <h5 className="truncate text-[13px] font-bold text-[#2a1b14]">{product.name}</h5>
                <p className="mt-0.5 text-[11px] font-semibold text-slate-500">{product.sold}</p>
              </div>
              <div className="text-right whitespace-nowrap pl-2">
                <p className="text-[13px] font-black text-[#9d4300]">{product.revenue}</p>
              </div>
            </div>
          ))
        ) : (
